...
export function validateRib(rib: string): ValidationResult {
  if (!rib) return { isValid: false, error: 'RIB is required' };
  if (!/^\d{20}$/.test(rib)) return { isValid: false, error: 'RIB must be exactly 20 digits' };
...
export function validateSiren(siren: string): ValidationResult {
  if (!siren) return { isValid: false, error: 'SIREN is required' };
  if (!/^\d{9}$/.test(siren)) return { isValid: false, error: 'SIREN must be exactly 9 digits' };
...
export function validateTaxId(taxId: string): ValidationResult {
  if (!taxId) return { isValid: false, error: 'Tax ID is required' };
  if (!/^\d{8,10}$/.test(taxId)) return { isValid: false, error: 'Tax ID must be 8-10 digits' };
  return { isValid: true };
}
...
export function validateDateFormat(date: string): ValidationResult {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return { isValid: false, error: 'Date must be YYYY-MM-DD format' };

  const [year, month, day] = date.split('-').map(Number);
  
  if (month < 1 || month > 12) return { isValid: false, error: 'Invalid month (must be 01-12)' };
  if (day < 1 || day > 31) return { isValid: false, error: 'Invalid day (must be 01-31)' };
...
export function validateSignatureDateFormat(date: string): ValidationResult {
  if (!date) return { isValid: true }; // Optional field
  if (!/^\d{10}$/.test(date)) return { isValid: false, error: 'Signature date must be DDMMyyHHmm format (10 digits, e.g., 1001261430)' };
  ...
  if (day < 1 || day > 31) return { isValid: false, error: 'Invalid day in signature date' };
  if (month < 1 || month > 12) return { isValid: false, error: 'Invalid month in signature date' };
  if (hour < 0 || hour > 23) return { isValid: false, error: 'Invalid hour (must be 00-23)' };
  if (minute < 0 || minute > 59) return { isValid: false, error: 'Invalid minute (must be 00-59)' };
...
export function validateIdentifier(value: string, type: string): ValidationResult {
  if (!value) return { isValid: false, error: `${type} is required` };
...
export function validateAmount(amount: number | string): ValidationResult {
  ...
  if (num > 9999999999.99999) return { isValid: false, error: 'Amount exceeds maximum value' };
...
export function validateTaxRate(rate: number | string): ValidationResult {
  const num = typeof rate === 'string' ? parseFloat(rate) : rate;

  if (isNaN(num)) return { isValid: false, error: 'Tax rate must be a number' };
  if (num < 0 || num > 1) return { isValid: false, error: 'Tax rate must be between 0 and 1 (0-100%)' };
...
export function validateIrcRate(rate: number | string): ValidationResult {
  const num = typeof rate === 'string' ? parseFloat(rate) : rate;

  if (isNaN(num)) return { isValid: false, error: 'IRC rate must be a number' };
  if (num < 0 || num > 0.1) return { isValid: false, error: 'IRC rate must be between 0 and 0.1 (0-10%)' };
...
export function validateInvoiceNumber(number: string): ValidationResult {
  if (!number) return { isValid: false, error: 'Invoice number is required' };
  if (!/^[A-Za-z0-9\-\/]+$/.test(number)) return { isValid: false, error: 'Invoice number can only contain letters, numbers, hyphens, and slashes' };
  if (number.length > 30) return { isValid: false, error: 'Invoice number is too long (max 30 characters)' };
  return { isValid: true };
}

/** Validate Company Name */
export function validateCompanyName(name: string): ValidationResult {
  if (!name || name.trim().length === 0) return { isValid: false, error: 'Company name is required' };
  if (name.length > 250) return { isValid: false, error: 'Company name is too long (max 250 characters)' };
  return { isValid: true };
}

/** Validate Street Address */
export function validateStreetAddress(street: string): ValidationResult {
  if (!street || street.trim().length === 0) return { isValid: false, error: 'Street address is required' };
  if (street.length > 100) return { isValid: false, error: 'Street address is too long (max 100 characters)' };
  return { isValid: true };
}

/** Validate City */
export function validateCity(city: string): ValidationResult {
  if (!city || city.trim().length === 0) return { isValid: false, error: 'City is required' };
  if (city.length > 50) return { isValid: false, error: 'City is too long (max 50 characters)' };
  return { isValid: true };
}

/** Validate Postal Code (alphanumeric, max 10 chars) */
export function validatePostalCode(code: string): ValidationResult {
  if (!code || code.trim().length === 0) return { isValid: false, error: 'Postal code is required' };
  if (!/^[A-Za-z0-9\s\-]+$/.test(code)) return { isValid: false, error: 'Postal code can only contain letters, numbers, spaces, and hyphens' };
  if (code.length > 10) return { isValid: false, error: 'Postal code is too long (max 10 characters)' };
  return { isValid: true };
}

/** Bulk Validation for Partner */
export function validatePartner(partner: any): ValidationResult[] {
