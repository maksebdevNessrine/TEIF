...
async function uploadPdf(invoiceId, userId, language, buffer) {
  try {
    ...
    const { error } = await client.storage.from(bucketName).upload(filePath, buffer, {
...
async function deletePdf(invoiceId, userId, language) {
  try {
    ...
    if (language) {
      ...
      const { data: files, error: listError } = await client.storage.from(bucketName).list(`invoices/${userId}/${invoiceId}`);
      if (listError) {
...
async function pdfExists(invoiceId, userId, language) {
  try {
    ...
    const bucketName = process.env.SUPABASE_STORAGE_BUCKET || "invoice-pdfs";
    const filePath = `invoices/${userId}/${invoiceId}/${language}.pdf`;
    const { data, error } = await client.storage.from(bucketName).list(`invoices/${userId}/${invoiceId}`);
    if (error) {
      return false;
    }
