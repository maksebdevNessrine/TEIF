...
export async function createInvoice(userId, invoiceData) {
    ...
    const invoice = await prisma.$transaction(async (tx) => {
        ...
        const lineIdMapping = {};
        const createdLines = await Promise.all(linesWithTotals.map((line) => tx.invoiceLine.create({
            data: {
...
export async function listInvoices(userId, options) {
    ...
    if (idOrderMap) {
        invoices = invoices.sort((a, b) => (idOrderMap.get(a.id) ?? Infinity) - (idOrderMap.get(b.id) ?? Infinity));
    }
    return {
        invoices,
        total,
