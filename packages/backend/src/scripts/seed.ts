...
export async function seedDatabase(): Promise<void> {
  ...
  const invoiceData: Array<{
    invoice: { id: string; metadata: any; globalDiscount?: number; operationNature: string; stampDuty: number };
  ...
  // ============================================
  // 3.b CREATE QUOTES (DOCUMENT CATEGORY = quote)
  // ============================================
  ...
  for (let i = 0; i < 5; i++) {
    ...
    quoteDate.setDate(quoteDate.getDate() - Math.random() * 90); // Last 3 months
    ...
    const quote = await prisma.invoice.create({
      data: {
        ...
        documentNumber: `QUO-2024-${(i + 1).toString().padStart(5, '0')}`,
        invoiceDate: quoteDate,
        dueDate: new Date(quoteDate.getTime() + 30 * 24 * 60 * 60 * 1000),
        userId: user.id,
        supplierId: isSupplier1 ? supplier1.id : supplier2.id,
        buyerId: Math.random() > 0.5 ? buyer1.id : buyer2.id,
        status: ['draft', 'sent'][Math.floor(Math.random() * 2)],
        operationNature: 'OP-SUPPLY',
        paymentMeans: paymentMethods[Math.floor(Math.random() * paymentMethods.length)],
        currency: 'TND',
        xmlContent: '<xml></xml>',
        metadata: {
          createdBy: user.email,
          notes: `Quote for ${isSupplier1 ? supplier1.name : supplier2.name}`,
        },
      },
    });
  ...
  // Add lines for quotes (simpler than invoices)
  for (const { quote } of quoteData) {
    const lineCount_ = Math.floor(Math.random() * 3) + 1; // 1-3 lines
    let quoteTotal = 0;
    let quoteTva = 0;
