  ...
  return `
      <PartnerDetails functionCode="${role}">
        ...
        <CtaSection>
          ...
          ${partner.email ? `<Communication><ComMeansType>I-102</ComMeansType><ComAdress>${partner.email}</ComAdress></Communication>` : ''}
        </CtaSection>
      </PartnerDetails>`;
};
...
export const generateTeifXml = (data: InvoiceData, minify: boolean = false): string => {
  ...
  const totalNetHt = subtotalNetHt - (data.globalDiscount || 0);
  ...
  const totalTtc = totalNetHt + totalFodec + totalTva + data.stampDuty;
...
<TEIF controlingAgency="TTN" version="1.8.8">
  ...
  <InvoiceBody>
    ...
    <Dtm>
      ...
      ${data.periodStart ? `<DateText format="ddMMyy" functionCode="I-34">${formatTtnDate(data.periodStart)}</DateText>` : ''}
      ${data.periodEnd ? `<DateText format="ddMMyy" functionCode="I-35">${formatTtnDate(data.periodEnd)}</DateText>` : ''}
    </Dtm>
    <PartnerSection>
    ...
    <InvoiceTax>
      ...
      ${totalFodec > 0 ? `<InvoiceTaxDetails><Tax><TaxTypeName code="I-162">FODEC</TaxTypeName><TaxDetails><TaxRate>1.0</TaxRate></TaxDetails></Tax><AmountDetails><Moa amountTypeCode="I-178" currencyCodeList="ISO_4217"><Amount currencyIdentifier="${data.currency}">${totalFodec.toFixed(3)}</Amount></Moa></AmountDetails></InvoiceTaxDetails>` : ''}
  ...
  <RefTtnVal><ReferenceTTN refID="I-88">${data.ttnReference}</ReferenceTTN></RefTtnVal>
